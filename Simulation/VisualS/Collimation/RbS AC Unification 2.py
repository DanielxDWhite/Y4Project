...
''' Variable Dance '''
IoIs = 2*(dip*E0/hbar)**2/G**2            # Intensity / Saturation Intensity
...
#                 A T O M    C O U N T E R          i know what I need to do here # what is it?# # # # # # # # # # # # # # # # # # # # # # ## # # # 
J = 10e20
Q = J*(Col_Gap/2)**2
q = Q*(Mm/Satoms)
print('q , Q =',q,Q)
...
for j in range(Natoms):
    for i in range(Nsteps):
        if (z_ < Z[j][i] < z__ and abs(Y[j][i]) < ymot and abs(V[j][i]) < capV):
            #Y[j],Z[j] = np.linspace(0,0,Nsteps), np.linspace(0,0.0001,Nsteps)    YMOT IS TOO BIG - CHANGE LATER
...
for j in range(Natoms):
    ...
    if NN == True:
        N0 += 1
    col = (0.1, float(j/(Natoms+1)+0.0001), 1-float(j/(Natoms+5)+0.0001))
    ax1.plot(Z[j],Y[j],linewidth=0.5, color = col)
    ax2.plot(Z[j],V[j],linewidth=0.5, color = col)
...
Ustr = 'Pinhole Diameter = {}mm\n Captured / Released = {}%\n Loading Rate = N/A'.format(Col_Gap*1000,round(N0*Mm/(Ssteps*Nn)*100,4)) # this is wrong
