...
y_vlin=y_vgen(Nvel)       # 20 velocities at each atom site
ylin=ygen(Natoms)
zs,vs,ts=[],[],[]
ys,yvs=[],[]
...
print(YVy(y_vlin, ylin, 2, 2))
print(YVy(y_vlin, ylin, 2, 2)[0])
...
for j in range(Natoms):    
    for k in range(Nvel):
        ...
        yi = YVy(y_vlin, ylin, k, j)[1]
        yvi = YVy(y_vlin, ylin, k, j)[0]
...
Y = np.reshape(ys, (Natoms*Nvel,Nsteps))
V = np.reshape(vs, (Natoms*Nvel,Nsteps))
Z = np.reshape(zs, (Natoms*Nvel,Nsteps))
#tt = np.array(ts)
...
nn=1
textstr = '     {}\nIntensity = {}mW/cm2\nDetuning = {}\n w/ Increment {}MHz\nE0 = {}    no. atoms = {} \nLength of Tube = {}cm\nMag Field Gradient = {}G/cm\n\n    Hole Diameter = {}mm\n    Hole Distance = {}cm\n    Pariticles lost = {}%'.format(d4,round(IrE*1000, 3),Da/1000000,d/1000000,E0,Natoms, round((z0-aa)*100,3),'Na', Col_Gap*1000, Col_zPos*100, (round((nn/Natoms)*100,3) ))#round(Grad*1000000,2))
